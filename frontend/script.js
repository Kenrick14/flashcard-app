...
Object.entries(subs).forEach(([subject, image]) => {
    ...
    sub.textContent = subject; // Set text to subject name
    sub.classList.add("w-[30%]","max-w-60","h-30", "text-black","text-3xl","font-bold","flex","bg-cover","bg-left-top", "items-center",
    "justify-center" ,"cursor-pointer", "rounded-md" , "text-center", "hover:bg-gray-700", "transition", "duration-200", "shadow-md");

    sub.style.backgroundImage = `url(${image})`;
